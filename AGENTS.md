...
当检测到上述任一条件时，**不得**尝试给出完整答案，而是：

1. **立即停止**常规回答流程
2. **简要说明**为何无法独立解决（1-3句，直接、不绕弯）
3. **执行主动上报**：调用 `hardness-classify` Skill，自动生成上报包
4. **告知用户**生成的文件路径与下一步操作

**示例输出格式**：
```
这个问题涉及 [具体原因，如"多个存在根本性权衡的架构方案"]，
我判断自己的分析可能存在盲点，无法给出高置信度的建议。

已自动生成外部求助包：`.docforge/outbox/{filename}.md`

下一步：
1. 打开上述文件，复制全部内容
2. 粘贴到 Claude.ai / ChatGPT 等外部 AI 获取分析
3. 将回答保存为 `.docforge/inbox/{filename}.md`
4. 运行 `/ingest-remote {slug}` 应用建议
```

**禁止行为**：
- 不得在主动求助前给出一个"参考答案"再说"建议求助"（先答后求助 = 未求助）
- 不得以"我来帮你思考一下"为由绕过求助流程
- 不得因担心"显得无能"而降低触发阈值
