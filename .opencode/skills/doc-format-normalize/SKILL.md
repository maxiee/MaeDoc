...
- 需要检查内容逻辑问题时（应改为调用 `doc-evaluate`）
...
| 指定规则不存在 | 使用 `question` 工具列出合法规则 ID，请用户重新指定 |

---

## 注意事项

1. **只修改格式，不改写内容**：本 Skill 不得修改文档的文字内容、术语、数据或结构；若发现内容问题，在报告末尾用"内容问题提示"单独列出，建议用户使用 `doc-evaluate` 或 `doc-iterate` 处理
