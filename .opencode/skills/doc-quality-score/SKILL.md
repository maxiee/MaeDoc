...
**正确调用时机**（满足以下任一条件时调用）：
- 需要对文档进行量化质量评估时（质量门检查、用户要求打分）
- 由 `doc-analyst` SubAgent 在质量分析流程中调用
- 作为 `/create` 或 `/iterate` 质量门阶段的一部分

**不应调用的情况**：
- 需要修改文档内容时（应改为调用 `doc-iterate`）
- 需要审阅具体问题位置时（应改为调用 `doc-review`，它提供更详细的定位）

**输入约束**：
- `document` 必须是已存在的文件路径或文档内容，不能是目录路径
- `weights` 各维度权重之和必须为 100%，否则回退为默认权重

**与其他 Skills 的协作**：
- 通常在 `doc-content-fill` 或 `doc-iterate` 完成之后调用
- 若总分 < 70，输出结果可直接传入 `doc-review` 做进一步问题定位
- 输出的"优先改进建议"可直接作为 `doc-iterate` 的 `feedback` 参数
...
3. 若提供自定义 `weights`，校验权重总和是否为 100%；若不合法，提示用户并使用默认权重
...
1. 检查文档是否有合理的层级结构（至少有 H1 和 H2），评估整体组织是否清晰
2. 额外加分项（最多 +3，不超过上限）：
   - 章节顺序合理：+1
...
4. **类型权重优先**：若用户通过 `weights` 参数自定义了各维度权重，则按自定义权重计算，不受默认值约束
...
2. 结构完整性评分：章节层级合理，无跳级 → **25/30**
3. 内容深度评分：9/11 章节内容充实，有 3 个 Mermaid 图 → **20/25**
4. 语言质量评分：发现 2 处冗余开场白（-2）、3 处中英文未加空格（-1.5）→ **16.5/20**，取整 **17/20**
5. 一致性评分："消息队列"与"MQ"混用（-2）、QPS 数据前后矛盾（-3）→ **10/15**
6. 可操作性评分：有下一步章节（+4）、有时间节点（+2）、无责任人（+0）→ **7/10**
7. 总分：25+20+17+10+7 = **79/100**（良好）
8. 优先改进建议：① 统一 QPS 数据（+3）② 添加测试策略章节（+2）③ 统一术语（+2）
9. 输出完整评分报告，建议修复优先级 1–2 的问题后再发布
