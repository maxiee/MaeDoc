...
| `doc_type` 未指定或模板不存在 | 尝试从文档内容推断；无法推断则按通用规范评分，在报告中注明"已按通用规范评分，如需更精确的结构评分请指定 `doc_type`" |
...
2. 加载 `docs/_templates/tech-design/type.json`（若存在），获取章节定义；若不存在则按通用规范评分
