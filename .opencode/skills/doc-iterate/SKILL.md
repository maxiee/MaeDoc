...
| 反馈未提供 | 请求用户描述具体的修改需求，不可继续 |
| 反馈过于模糊（如"改好一点"） | 列出 3-5 个可能的解读，请用户选择或澄清，再执行 |
| `doc_type` 未指定 | 尝试从文档内容推断；无法推断则按通用规范迭代，在报告中注明 |
| 要删除的章节是必需章节 | 警告用户该章节为文档类型的必需章节，确认是否仍要删除 |
| 反馈涉及需要外部信息的内容（如竞品数据） | 在对应位置标注 `[待确认：{需要的信息}]`，在"未处理反馈项"中注明 |
| 文档冲突（修改 A 与修改 B 互相矛盾） | 列出冲突，请用户裁决，不自行决策 |
