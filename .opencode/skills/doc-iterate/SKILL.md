...
- 需要量化评分或问题审阅时（应改为调用 `doc-evaluate`）
...
**与其他 Skills 的协作**：
- 通常在 `doc-evaluate` 之后调用（基于评分结果执行改进）
...
- "iterate 这个文档"
- "帮我优化一下这部分内容"
- 或由 `/iterate` 命令内部调用，或由 `doc-evaluate` 输出的后续建议触发
...
| `feedback` | 是 | 用户反馈：自然语言描述（如"补充威胁模型分析"）或结构化问题列表（如 `doc-evaluate` 输出的 P0/P1 问题） |
...
2. 解析 `feedback`，识别反馈类型：
   ...
   - **结构化问题列表**（来自 `doc-evaluate`）：按 P0→P1→P2 优先级排序处理
...
若输出的"未处理的反馈项"中存在任何条目，**必须**调用 `todo-append` Skill 将其追加到 `docs/TODO.md`：

- 可为每个未处理项单独记录一条 TODO，或合并为一条（视条目数量决定）
- `description`：具体描述未处理的反馈内容是什么、期望做什么
- `source`：`由 doc-iterate Skill 在迭代 {文档路径} 时发现`
- `priority`：`中`
- `background`：注明未处理原因（如"需要用户补充数据""超出当前文档范围""反馈存在歧义"），以及解决所需的前提条件

**若所有反馈均已处理（"未处理的反馈项"为空），跳过此步骤。**
...
> **文档**：{文档标题或文件路径}
> **迭代时间**：{当前日期}
> **反馈来源**：{自然语言 / doc-evaluate P0-P1 问题清单}
> **迭代范围**：{受影响的章节列表}
...
{根据迭代结果给出 1-3 条建议，如：}
1. {如：建议重新运行 `doc-evaluate` 评估迭代后的质量提升}
...
| `doc_type` 未指定或模板不存在 | 尝试从文档内容推断；无法推断则按通用规范迭代，在报告中注明 |
| 要删除的章节 | 使用 `question` 工具警告用户，确认是否仍要删除 |
...
5. 生成迭代报告：变更摘要（新增 1、修改 2、修复 3）+ 完整修改后文档 + 后续建议（建议重新运行 `doc-evaluate`）
