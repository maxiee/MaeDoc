...
| 要删除的章节是必需章节 | 使用 `question` 工具警告用户（若模板存在），确认是否仍要删除 |
...
2. 尝试加载 `docs/_templates/tech-design/type.json`，若存在则确认"安全与隐私考量"为必需章节
3. 将 3 条反馈映射为操作：
   - 操作 1：`ADD_SECTION "安全与隐私考量"`（位置：在"测试策略"之前）
