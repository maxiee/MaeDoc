...
若 `docs/index.md` 存在，读取其内容：

1. 了解现有文档结构和已覆盖的主题
2. 识别与当前想法主题相关的已有文档（最多 2 个），读取其首段或摘要
3. 在生成大纲时：
   - 避免重复已有文档的核心内容（在大纲中通过相对路径链接引用，而非重写）
   - 在大纲的"相关文档"字段标注可参考的已有文档路径
   - 利用已有文档的术语定义保持术语一致性

若 `docs/index.md` 不存在，跳过此步骤，继续步骤 2。
...
**若大纲"需要用户确认的问题清单"中存在"必须回答"项**（即影响核心结构的关键信息缺失），**必须**调用 `todo-append` Skill 记录：

- `description`：列出哪些关键信息尚待确认（引用"必须回答"问题列表中的具体内容）
- `source`：`由 doc-outline-generate Skill 在生成 {output_file 或当前文档} 大纲时发现`
- `priority`：`高`
- `background`：说明这些信息的缺失对大纲核心结构的影响（影响哪些章节、当前使用了什么假设值）

**若"必须回答"列表为空（所有关键信息已具备），跳过此步骤。**
