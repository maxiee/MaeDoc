...
2. 确认目标文档类型：
   ...
   - 如未明确指定 → 基于意图推断最匹配的类型，使用 `question` 工具向用户确认：
     - **标题**：选择文档类型
     - **问题**：根据你的描述，我建议使用「{类型名称}」文档类型（适合：{类型描述}）。是否继续，或希望换一个类型？
     - **选项**：使用推荐类型「{类型名称}」/ {其他可用类型列表}
...
**若提供了 `output_file` 参数**：在输出大纲内容后，立即将大纲内容写入 `output_file` 指定的文件（若文件不存在则创建，若已存在则覆盖）。写入完成后输出：
...
| 场景 | 处理方式 |
|------|---------|
| 文档类型不明确 | 使用 `question` 工具推断并向用户确认，不强制阻塞 |
