...
- 若有对应 `doc_type` 且模板存在：必需章节（`required: true`）是否全部存在？章节顺序是否与类型定义一致？
- 若无对应模板：文档是否有明确的层级结构（标题 H1→H2→H3 不跳级）？
...
2. 尝试读取 `docs/_templates/tech-design/type.json`，若不存在则切换为通用规范审阅
3. 执行五维度审阅：
