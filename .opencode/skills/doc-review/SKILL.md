...
**正确调用时机**（满足以下任一条件时调用）：
- 需要定位文档具体问题（位置、类型、严重级别）时
- 由 `doc-analyst` SubAgent 在总分 < 70 时调用，识别 P0/P1 问题
- 用户明确要求"审阅文档"或"找出问题"时

**不应调用的情况**：
- 需要量化评分时（应改为调用 `doc-quality-score`，它提供 0-100 分）
- 需要修改文档内容时（应改为调用 `doc-iterate`）

**输入约束**：
- `document` 必须是已存在的文件路径或文档内容
- 若不提供 `focus_areas`，则执行全维度审阅（5 个维度）

**与其他 Skills 的协作**：
- 通常在 `doc-quality-score` 之后调用（当分数不达标时做深度分析）
- 输出的 P0/P1 问题列表可直接作为 `doc-iterate` 的 `feedback` 参数
...
3. 确认审阅维度：按 `focus_areas` 参数或默认执行全维度审阅
...
审阅报告生成后，`/review` 流程仅输出报告，不立即修复问题。若报告中存在 P0 或 P1 问题，**必须**调用 `todo-append` Skill 将其记录到 `docs/TODO.md`：

- **P0 问题**：每个单独记录一条 TODO，`priority = 高`
- **P1 问题**：可合并为一条 TODO（或分别记录），`priority = 中`
- **P2 问题**：可选择性记录，`priority = 低`
- `source`：`由 doc-review Skill 在审阅 {文档路径} 时发现`
- `background`：引用报告中该问题的具体描述与修改建议

**若报告中仅有 P2 问题或无任何问题，此步骤可跳过。**
...
1. 读取文档，识别章节结构
2. 执行五维度审阅：
   - 结构完整性：检查标题层级、章节是否完整 → 发现"测试策略"章节缺失（P0）
...
3. 输出审阅报告：2 个 P0、3 个 P1、2 个 P2，总体评价：框架清晰但有结构缺失和数据不一致需优先修复
