...
description: 检查文档是否符合其文档类型定义的结构要求，包括必需章节存在性、章节顺序、断链检测、重复内容检测，以及 docs/ 目录级别的结构合规检查（行数限制、index.md 导航规范、未注册文档），输出结构审计报告（缺失项 + 问题定位 + 修复建议）
---
...
> **版本**：1.1.0
> **类型**：instruction
> **用途**：检查文档是否符合其文档类型定义的结构要求，以及 docs/ 目录是否遵循 `maedoc/docs-structure-standard.md` 定义的结构标准，输出审计报告
...
**此步骤完成后，跳过步骤 1-5，直接执行步骤 6（生成汇总修复清单）。**
...
2. 确定审计模式：
   - 若提供了 `doc_type` 且 `docs/_templates/{type_id}/type.json` 存在 → **模板合规模式**：加载 type.json，对比必需章节与期望顺序
   - 若 `doc_type` 未提供，或对应 type.json 不存在 → **通用结构模式**：仅检查文档自身的结构完整性（层级、断链、重复），不与外部模板对比
3. 模板合规模式下，从 `type.json` 提取**期望章节树**：
   - 必需章节（`required: true`）
   - 可选章节（`required: false`）
   - 子章节定义（`subsections`）
...
| `doc_type` 未指定或 type.json 不存在 | 切换为通用结构模式，仅执行断链检测、重复内容检测和标题层级检查，在报告中注明"已按通用结构模式审计" |
...
**Skill 行为**：
1. 读取文档，提取实际章节树
2. `doc_type` 未指定且 `docs/_templates/` 目录不存在 → 切换为通用结构模式
3. 必需章节检查：跳过（无模板可对照），报告中注明"通用结构模式"
4. 章节顺序检查：跳过（无模板可对照）
5. 断链检测：发现 1 处内部锚点 `#architecture-diagram` 无对应标题 → 记录为 ❌ 错误
6. 重复内容检测：未发现重复问题
7. 输出报告：通用结构模式，1 个错误（断链）、1 个警告（章节顺序无法与模板对照）
