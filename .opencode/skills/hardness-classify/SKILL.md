...
**正确调用时机**（满足以下任一条件时调用）：
- 检测到 AGENTS.md §9.1 任一触发条件（H1-H6）时，在正式回答前主动调用
- 用户显式要求"做硬度评估"时

**不应调用的情况**：
- 问题明确、置信度高时（直接回答，不需要评估）
- 已知答案在常规写作规范范围内时

**输入约束**：
- `question` 必须是具体的决策点或问题，不能是模糊描述
- 参数均可由 AI 从对话上下文中自动提取，不需要用户手动提供

**与其他 Skills 的协作**：
- 独立 Skill，不依赖其他 Skills
- HIGH 等级时生成外部求助包，由 `/ingest-remote` 命令处理返回结果
...
### 步骤 2.5（仅适用于 MEDIUM 等级）：渐进式求助

若等级为 MEDIUM，不立即生成上报包，而是使用 `question` 工具向用户提供选择：

- **标题**：如何继续
- **问题**：我对这个问题有初步分析，但存在明显不确定性（评估 {N}/12，MEDIUM 等级）。请选择处理方式：
- **选项**：
  1. 给出初步建议并标注置信度（[MED]/[LOW]），我来判断是否采纳（Recommended）
  2. 先回答我 1-2 个关键问题，让建议更准确
  3. 生成外部求助包，咨询外部 AI

根据用户选择：

- **选择 1**：继续正常回答流程，对不确定内容显式标注 `[MED]` 或 `[LOW]`，并在回答末尾注明关键不确定点
- **选择 2**：使用 `question` 工具提出最关键的 1-2 个追问，收到回答后继续回答流程（信息增益足够时升至 [MED] 置信度）
- **选择 3**：跳转步骤 3，执行 HIGH 等级的主动上报流程

---

### 步骤 3：若等级为 HIGH，执行主动上报

> 若等级为 LOW，跳过本步骤，返回正常回答流程。
> 若等级为 MEDIUM，已由步骤 2.5 处理，用户选择"选项 3"时才执行本步骤。
