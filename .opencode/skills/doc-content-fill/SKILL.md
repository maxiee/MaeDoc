...
4. **写回文件（若提供了 `output_file`）**：将该章节的实际内容替换文件中的 `*（填充中...）*` 占位内容，将已填充的章节写入文件。每章节填充完毕后立即执行写入，不等待所有章节完成。写入后输出：
   ```
...
1. 所有章节填充完成后，将完整文档（含文档头部元信息，状态从"填充中..."更新为"草稿"）写入 `output_file`（若已提供），或在上下文中输出完整文档
