...
4. **记录遗留事项为 TODO**：若填充摘要中"遗留待确认项"列表非空（即存在未解决的 `[待确认: ...]` 占位，或有 🔴 低信心章节），**必须**调用 `todo-append` Skill 将遗留事项追加到 `docs/TODO.md`：
   - 可将所有遗留项合并为一条 TODO，也可为关键项各记一条
   - `description`：说明哪个章节有哪些待确认内容，以及后续需要补充什么
   - `source`：`由 doc-content-fill Skill 在填充 {output_file} 时发现`
   - `priority`：`中`
   - `background`：注明当前假设值及其对文档准确性的影响范围
