...
命令主体完成后，逐条检查以下情况是否发生：

| # | 检查项 | 是否发生 |
|---|--------|:----:|
| 1 | 文档中存在新增或未解决的 `[待确认]` 标注 | 是 / 否 |
| 2 | 迭代过程中发现某文档/章节需要更新，但不在本次任务范围内 | 是 / 否 |
| 3 | 某步骤因时机不对或范围越界被主动跳过 | 是 / 否 |
| 4 | 发现内容矛盾、格式问题或信息缺失，修复超出当前范围 | 是 / 否 |
| 5 | 用户在对话中提到了需要后续跟进的事项，本次未处理 | 是 / 否 |
