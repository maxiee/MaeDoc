...
### 阶段 5：TODO 录入检查（必做，不可跳过）

文档写入完成后，逐条检查以下情况是否发生：

| # | 检查项 | 是否发生 |
|---|--------|:----:|
| 1 | 部分外部 AI 建议被跳过，其中有需要后续专项处理的有价值建议 | 是 / 否 |
| 2 | 文档中存在新增的 `[待确认]` 标注需要后续确认 | 是 / 否 |
| 3 | 应用建议时发现某关联文档也需要更新，但不在本次范围内 | 是 / 否 |
| 4 | 用户在对话中提到了需要后续跟进的事项，本次未处理 | 是 / 否 |
| 5 | 发现需要跨多文档协调的内容，当前时机不适合处理 | 是 / 否 |
...
[阶段 4] 生成修改后文档 → 展示变更摘要 → question 工具最终确认
    ...
    └─ 放弃 → 输出"已取消"，终止
    ▼
[阶段 5] TODO 录入检查（必做）→ 逐条判断 5 项触发条件
