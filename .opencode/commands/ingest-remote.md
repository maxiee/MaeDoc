...
|---|---|:---:|
| 1 | 被跳过的建议中，有因"范围太大"或"需要专项处理"而跳过的有价值建议？（区别于因质量不佳而跳过） | 是 / 否 |
| 2 | 应用建议后，文档中新增了 `[待确认]` 标注？（完成摘要已统计数量） | 是 / 否 |
| 3 | 应用建议时，注意到关联文档也需要更新，但本次未处理？ | 是 / 否 |
| 4 | 用户在逐条审查过程中提及了本次未处理的想法或后续意图？ | 是 / 否 |
| 5 | 外部 AI 的建议中涉及跨多文档协调的内容，当前时机不适合一并处理？ | 是 / 否 |
...
[阶段 4] 生成修改后文档 → 展示变更摘要 → question 工具最终确认
    ...
    └─ 放弃 → 输出"已取消"，终止
    ▼
[阶段 5] TODO 录入（必做）→ 回顾心智便签，逐条判断 5 项具体触发线索
