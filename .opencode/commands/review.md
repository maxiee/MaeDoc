...
**主线模式**：专注高质量地完成文档审阅，输出有价值的综合报告。

**后台扫描模式**：以"文档生态守护者"的视角，持续留意以下改进信号。每次信号出现时，**即时在心智便签中标记**（不打断主流程，供末尾"TODO 录入"阶段处理）：

- 审阅中发现的、超出当前文档范围的问题（关联文档需要同步、跨文档概念不一致等）
- 报告中指出的、需要专项操作处理的重大问题（大规模重写、结构重组等）
- 用户在提问互动中透露的后续计划或关切，本次审阅未覆盖
...
- `document`：文档路径
- `focus_areas`：阶段 1.1 用户指定的审阅重点（全面审阅则不传）
...
> 💡 **后台扫描触发点**：内容审阅完毕后，在进入阶段 5 之前，**立即扫描一遍心智便签**：审阅中发现的超出本文档范围的问题（如其他文档需要同步）、报告中会列出的"适合后续专项处理"的重大问题——这些是接下来"TODO 录入"最重要的来源，现在正是记录它们的时机。
...
|---|---|:---:|
| 1 | 审阅报告"必须处理"或"重要改进"中，有适合通过专项 `/iterate` 处理的重大问题？ | 是 / 否 |
| 2 | 审阅中注意到某个关联文档也需要同步更新或修正，但本次审阅范围未覆盖？ | 是 / 否 |
| 3 | 审阅过程中发现了需要用户提供更多信息才能确认的问题，但本次未继续追踪？ | 是 / 否 |
| 4 | 用户在阶段 1 或提问互动中透露了后续计划或关切，本次审阅未覆盖？ | 是 / 否 |
| 5 | 审阅发现的问题涉及跨多文档协调（概念不一致、断裂引用等），需要专项处理？ | 是 / 否 |
...
> 提示：审阅几乎总会发现超出当前文档范围的改进机会。若确实没有，解释会很自然简短；若难以解释，说明那里确有值得记录的事项。
