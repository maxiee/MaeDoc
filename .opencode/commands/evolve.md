...
将被跳过的操作记录为 TODO（读取 `.opencode/skills/todo-append/SKILL.md`，按步骤追加）。
- 内容操作跳过：以操作目标文件为维度合并，`priority = 中`
- 中/高风险操作跳过：以操作类型为维度合并，`priority = 高`

**专项检查 B：操作执行失败**

为每个失败操作单独记录一条 TODO，`priority = 高`。

**通用触发线索**（逐条判断）：

1. 全库扫描识别出改进点但未纳入变更计划？
2. 执行中发现需要专项内容质量处理的问题？
3. 用户提及了本次未覆盖的后续演进方向？

**对"是"项**：读取 `.opencode/skills/todo-append/SKILL.md`，按步骤追加到 `docs/TODO.md`。

**完成声明**：有记录写 `已记录 TODO：{编号列表}`；零 TODO 逐条说明为何未触发。
...
[阶段 5] 输出完成摘要
    │
    ▼
[阶段 5.5] TODO 录入（必做）→ 专项检查 A/B + 通用检查 → 追加 TODO 或说明为何无项
```
