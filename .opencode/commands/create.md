...
> 基于"写→诊断→调整"反馈循环。加载 `.opencode/skills/quality-gate/SKILL.md`，按其步骤执行。

**target_file**：单文件模式为 `output_file` 的绝对路径，多文件模式为 `base_dir/index.md` 的绝对路径。

> **多文件模式说明**：多文件模式下 `output_file` 已在阶段 3 被删除，以 `base_dir/index.md` 为评估锚点。
...
[阶段 3] task 工具调用 doc-writer SubAgent
    ...
    ▼
[阶段 4] 质量门检查（必做）— 加载 quality-gate Skill
    │   → 单文件：对 output_file 评分
    │   → 多文件：对 base_dir/index.md 评分
    │   ├─ PASS → 继续阶段 5
...
[阶段 5] 输出完成摘要
    ▼
[阶段 6] 主 Agent 更新 docs/index.md
    ▼
[阶段 7] TODO 录入（必做）→ 判断 5 项触发线索 → 追加 TODO 或说明为何无项
```
