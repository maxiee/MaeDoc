...
### 阶段 7：TODO 录入检查（必做，不可跳过）

命令主体完成后，逐条检查以下情况是否发生：

| # | 检查项 | 是否发生 |
|---|--------|:----:|
| 1 | 创建过程中发现某章节需要补充，但补充超出当前文档范围 | 是 / 否 |
| 2 | 某步骤因信息不足或范围越界被主动跳过 | 是 / 否 |
| 3 | 发现相关文档需要更新或同步，但不在本次任务范围内 | 是 / 否 |
| 4 | 用户在对话中提到了需要后续跟进的事项，本次未处理 | 是 / 否 |
| 5 | 发现需要跨多文档协调的复杂操作，当前时机不适合 | 是 / 否 |
...
[阶段 6] 更新 docs/index.md → 追加文档地图条目（单文件链接文档；多文件链接 base_dir/index.md）
    │
    ▼
[阶段 7] TODO 录入检查（必做）→ 逐条判断 5 项触发条件
