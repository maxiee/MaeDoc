...
> **目标**：在生成大纲之前，通过持续追问充分了解用户需求，积累的问答记录将作为 `enriched_context` 传入后续阶段。
...
**追问方向参考**（根据文档类型和已有信息灵活选择，不必按顺序）：

| 维度 | 示例问题 |
|------|---------|
| 目标读者 | 这篇文档主要写给谁看？（团队内部 / 外部开发者 / 管理层 / 其他） |
| 核心目标 | 你最希望读者读完后能做到什么？ |
| 内容边界 | 有哪些内容**一定要包含**？有哪些**不需要覆盖**？ |
| 已有素材 | 你有现成的草稿、参考文档或代码片段可以提供吗？ |
| 风格偏好 | 偏好正式严谨的技术风格，还是轻松易读的科普风格？ |
...
[阶段 1.5] 确定输出文件路径 → 创建 docs/ 目录（如需）→ 告知用户文件路径
    │
    ▼
[阶段 2] doc-outline-generate（传入 enriched_context）→ 输出大纲 → 立即写入 output_file → 用户确认
