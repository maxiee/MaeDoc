...
大纲生成完毕后，使用 `question` 工具呈现大纲并请求确认：

- **标题**：确认文档大纲
- **问题**：大纲已生成（见上方输出），请选择下一步操作：
- **选项**：
...
- **选择 1（确认）**：继续阶段 3
- **选择 2（修改）**：收集用户修改意见，更新大纲，再次使用 `question` 工具确认
- **选择 3（重新生成）**：使用 `question` 工具收集更多背景信息，重新调用 `doc-outline-generate`，再次确认
